    ...
    ["ls", ..outputs] ->
      {
        ...
        case output {
          "dir " <> dir -> Ok(Dir(dir))
          _ -> {
            assert Ok(#(size, name)) = string.split_once(output, " ")
...
fn dir_sizes(l: List(#(List(String), List(FS)))) {
  ...
  let sorted =
    list.sort(
      l,
      fn(e1, e2) { int.compare(list.length(e2.0), list.length(e1.0)) },
    )
  ...
  cum_size + case fs {
    ...
    Dir(subdir) -> {
      ...
      assert Ok(subdir_size) = map.get(tree, [subdir, ..elem.0])
      subdir_size
    }
  }
}
...
pub fn pt_2(input: String) {
  use sizes <- solve(input)
  assert Ok(total) = map.get(sizes, [])

  let delete_at_least = total - { 70000000 - 30000000 }
